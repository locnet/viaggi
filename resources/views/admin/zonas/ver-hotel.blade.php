        ...
        @foreach($hotel as $h)
            @foreach($h->getNodes($parserNodes['nod'],$parserNodes['subnod']) as $hotelData)
                <div class="col-md-12 col-xs-12 login">
                    <p><a href="{{ url('admin/destino/'.$hotelData['destination'].'/zonas') }}">
                    ...
                    <h3 class="lato-300 blue">
                        {{ strtoupper($hotelData['building_type_name']).' '.$hotelData['name'] }}                       
                        
                        <span style="color: #898989; font-size: 80%">
                        , Direccion: {{ $hotelData['address'].', '.$hotelData['city'] }}
